...
class SiswaBelumAktivasiExport implements FromCollection, WithHeadings, WithColumnWidths, WithStyles
{
    public function collection()
    {
        $data = KartuUjian::with(['ujian', 'siswa', 'aktivasiKartu'])
            ...
            ->where(function ($query) {
                // Pilih siswa yang aktivasi statusnya false
                $query->whereHas('aktivasiKartu', function ($subQuery) {
                    $subQuery->where('status_aktivasi', false);
                })
                // ATAU siswa yang sama sekali belum memiliki record di tabel aktivasiKartu
